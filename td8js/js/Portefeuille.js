...
export class portefeuille {
    ...
    ajouter(type, montant, taux) {
        document.getElementById("error").innerText = " ";
        if (montant > 0) {
            let index = this.recherche(type);
            if (index !== -1) {
        ...
        if (this._table[this.recherche(nomDev)]) {
            if (nb <= this._table[this.recherche(nomDev)].montant) {
                if (this.recherche(nomDev) != -1) {
                    ...
                    document.getElementById("error").innerText = "Erreur : Vous n'avez pas cette monnaie";
                }
            }
            else {
                document.getElementById("error").innerText = "Erreur : Vous ne pouvez pas retirer autant";
            }
        }
        else {
            document.getElementById("error").innerText = "Erreur : Il n'y a pas de " + nomDev;
        }
    }
    isExist(nomDev) {
        if (this.recherche(nomDev) != -1) {
