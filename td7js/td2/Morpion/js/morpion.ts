...
function recommence() {
    ...
    if (Number.isNaN(taille) || taille < MIN_GRILLE || taille > MAX_GRILLE) {
        ...
        let table = document.getElementById("table_morpion") as  HTMLTableElement;
...
function clicBouton(uneCase : HTMLElement, y: number, x: number) {
...
function aGagne(symbole : string, y: number, x: number) {
